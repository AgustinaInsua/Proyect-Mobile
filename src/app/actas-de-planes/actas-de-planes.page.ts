...
import {
  FIELDS_TABLE_DEBTS,
  FIELDS_TABLE_PAYMENTS,
  FIELDS_TABLE_RECORD,
} from '../model/mock-fieldsTableRecord';
...
export class ActasDePlanesPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apiService
      .get(ApiService.apiURLCompanies + '/64640/actas/56540')
      .subscribe({
        next: (records) => {
    ...
    this.apiService.get(ApiService.apiURLCompanies + '/64640').subscribe({
      next: (companies) => {
    ...
    this.items = [
      {
        ...
        items: [
          ...
          },
          {
            label: 'Actas',
            items: [
              { label: 'Acta', command: (event) => {} },
              { label: 'Acta Compactada', command: (event) => {} },
            ],
          },
          {
            label: 'Certificados',
            items: [
              { label: 'Certificado', command: (event) => {} },
              { label: 'Certificado Compactado', command: (event) => {} },
            ],
          },
          {
            label: 'PDF Adhesion',
        ...
        items: [
          {
            label: 'Asociar',
            items: [
              { label: 'Fecha Calc. Int.', command: (event) => {} },
              { label: 'Fecha de Pago', command: (event) => {} },
              {
                label: 'Cheque',
                command: (event) => {
              ...
              { label: 'CBU General', command: (event) => {} },
            ],
          },
          {
            label: 'Aprobar',
            items: [
              ...
              { label: 'Fecha de Acreditación', command: (event) => {} },
            ],
          },
          {
          ...
          {
            label: 'Eliminar',
            items: [{ label: 'Fecha de Pago', command: (event) => {} }],
          },
        ],
      },
    ];
